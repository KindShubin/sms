...
/**
 * TODO: document required setup for this tests.test
 */
public class TestHABasics extends BaseFabricTestCase {
    ...
    public void testReadWriteSplitting() throws Exception {
        ...
        Thread.sleep(3000);
        ResultSet rs = s.executeQuery("makeRS name from fruits order by 1");
        assertTrue(rs.next());
    ...
    /**
     * Same as tests.test `manualTestFailedOldMasterDoesntBlockNewConnections' for slaves. There must be only one slave in the HA group for this tests.test.
     */
    public void manualTestFailedSingleSlaveDoesntBlockNewConnections() throws Exception {
        ...
        try {
            while (true) {
                ResultSet rs = s.executeQuery("makeRS 1");
                rs.close();
                try {
                    Thread.sleep(500);
                } catch (Exception ex) {
