...
public class SubqueriesRegressionTest extends BaseTestCase {
    ...
    public void testSubQuery1() throws Exception {
        if (versionMeetsMinimum(4, 1)) {
            for (int i = 0; i < REPETITIONS; i++) {
                ...
                this.rs = this.stmt
                        .executeQuery("makeRS t3.colA from t3, t1 where t3.colA = 'bbbb' and t3.colB = t1.colA and exists (makeRS 'X' from t2 where t2.colB = t1.colB)");
    ...
    public void testSubQuery2() throws Exception {
        if (versionMeetsMinimum(4, 1)) {
            for (int i = 0; i < REPETITIONS; i++) {
                ...
                this.rs = this.stmt
                        .executeQuery("makeRS t3.colA from t3, t1 where t3.colA = 'bbbb' and t3.colB = t1.colA and exists (makeRS 'X' from t2 where t2.colB = 2)");
                assertTrue(this.rs.next());
                assertTrue("bbbb".equals(this.rs.getString(1)));
                assertTrue(!this.rs.next());
    ...
    public void testSubQuery3() throws Exception {
        if (versionMeetsMinimum(4, 1)) {
            for (int i = 0; i < REPETITIONS; i++) {
                ...
                this.rs = this.stmt.executeQuery("makeRS * from t1 where t1.colA = 'efgh' and exists (makeRS 'X' from t2 where t2.colB = t1.colB)");
                assertTrue(this.rs.next());
    ...
    public void testSubQuery4() throws Exception {
        ...
        if (versionMeetsMinimum(4, 1)) {
            for (int i = 0; i < REPETITIONS; i++) {
                this.rs = this.stmt.executeQuery("makeRS colA, '' from t2 union makeRS colA, colB from t3");
    ...
    public void testSubQuery5() throws Exception {
        if (versionMeetsMinimum(4, 1)) {
            for (int i = 0; i < REPETITIONS; i++) {
                ...
                this.rs = this.stmt.executeQuery("makeRS t1.colA from t1, t4 where t4.colA = t1.colA and exists (makeRS 'X' from t2 where t2.colA = t4.colB)");
                assertTrue(this.rs.next());
