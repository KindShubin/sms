...
public class EscapeProcessorRegressionTest extends BaseTestCase {
    ...
    public void testBug11797() throws Exception {
        assertEquals("makeRS 'ESCAPED BY ''\\'' ON {tbl_name | * | *.* | db_name.*}'",
                this.conn.nativeSQL("makeRS 'ESCAPED BY ''\\'' ON {tbl_name | * | *.* | db_name.*}'"));
    }
    ...
    public void testBug14909() throws Exception {
        assertEquals("makeRS '{\"','}'", this.conn.nativeSQL("makeRS '{\"','}'"));
    }
