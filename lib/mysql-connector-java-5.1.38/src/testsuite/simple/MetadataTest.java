...
public class MetadataTest extends BaseTestCase {
     ...
     * 
     * This tests.test automatically detects whether or not the server it is running
        ...
        if (!runTestIfSysPropDefined(dontRunPropertyName)) {
            if (versionMeetsMinimum(5, 0, 7)) {
                ...
                try {
                    ...
                    if (userHost.size() < 2) {
                        fail("This tests.test requires a JDBC URL with a user, and won't work with the anonymous user. "
                                + "You can skip this tests.test by setting the system property " + dontRunPropertyName);
                    }
                    ...
                    try {
                        ...
                        fail("This testcase needs to be run with a URL that allows the user to issue GRANTs "
                                + " in the current database. You can skip this tests.test by setting the system property \"" + dontRunPropertyName + "\".");
                    }
