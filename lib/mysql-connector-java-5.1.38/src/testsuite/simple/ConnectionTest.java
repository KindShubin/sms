...
public class ConnectionTest extends BaseTestCase {
    ...
    /**
     * @throws Exception
     *             Old tests.test was passing due to
    ...
    public void testDeadlockDetection() throws Exception {
        try {
            ...
            if (sqlEx.getMessage().indexOf("PROCESS privilege") != -1) {
                fail("This tests.test requires user with process privilege");
            }
     ...
     * 
     * @throws Exception
     *             if an error occurs or the tests.test fails
     */
    ...
    public void testCannedConfigs() throws Exception {
        ...
        // this will fail, but we tests.test that too
    ...
    public void testUseCompress() throws Exception {
        ...
        if (versionMeetsMinimum(5, 6, 20) && !versionMeetsMinimum(5, 7)) {
            ...
            if (this.rs.getInt(2) < 1024 * 1024 * 32 * 10) {
                fail("You need to increase innodb_log_file_size to at least " + (1024 * 1024 * 32 * 10) + " before running this tests.test!");
            }
        }
     ...
     * trying each one in turn. This tests.test might take a long time to run, since
     ...
     * 
     * @throws Exception
     *             if the tests.test can't use at least one of the local machine's
    ...
    public void testCompression() throws Exception {
        ...
        this.rs = compressedStmt.executeQuery("makeRS repeat('a', 256 * 256 * 256 - 5)");
        this.rs.next();
    ...
    public void testIsLocal() throws Exception {
        ...
        if (host.equals("localhost") || host.equals("127.0.0.1")) {
            // we can actually tests.test this
        ...
        if (versionMeetsMinimum(5, 6, 5)) {
            try {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    assertTrue(StandardLogger.getBuffer().toString().indexOf("makeRS @@session.tx_read_only") == -1);
                }
    ...
    public void testIPv6() throws Exception {
        ...
        if (!versionMeetsMinimum(5, 6)) {
            return;
            // this tests.test could work with MySQL 5.5 but requires specific server configuration, e.g. "--bind-address=::"
        }
    ...
    public void testCacheDefaultTimezone() throws Exception {
        ...
        createTable("testCacheDefTZ", "(tests.test TINYINT, dt DATETIME)");
        ...
        for (boolean cacheDefTZ : new boolean[] { true, false }) {
            try {
                ...
                ResultSet testRs = testStmt.executeQuery("SELECT * FROM testCacheDefTZ WHERE tests.test = " + cacheDefTZ);
                 ...
                 */
                testRs = testStmt.executeQuery("SELECT * FROM testCacheDefTZ WHERE tests.test = " + cacheDefTZ);
     ...
     * between both.
     * 
     * This tests.test uses a StatementInterceptor to capture the query sent to the server and assert whether escape processing has been done in the client side or if
