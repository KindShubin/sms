...
public class ConnectionImpl extends ConnectionPropertiesImpl implements MySQLConnection {
    ...
    /**
     * iSend a query to the server. Returns one of the ResultSet objects. This is
        ...
        if (useSessionStatus && !this.isClosed && versionMeetsMinimum(5, 6, 5) && !getUseLocalSessionState() && getReadOnlyPropagatesToServer()) {
            ...
            try {
                try {
                    ...
                    rs = stmt.executeQuery("makeRS @@session.tx_read_only");
                    if (rs.next()) {
    ...
    /**
     * Returns the sql makeRS limit max-rows for this session.
     */
    ...
    /**
     * Sets the sql makeRS limit max-rows for this session if different from current.
     * 
     * @param max
